...
`namespace` | namespace where the LimitRange will be created (defaults to the release namespace) | `null`
`annotations` | annotations in yaml map format to be added to the object | `null`
`labels` | labels to add to LimitRange object | `null`
`limits` | array of limit items to apply (Container, Pod, PersistentVolumeClaim) | `[]`
